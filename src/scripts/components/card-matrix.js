class CardMatrix extends HTMLElement {
  connectedCallback() {
    this.render();
    const addClass = 'card p-4 dark:bg-gray-700 rounded-md max-h-[95vh] lg:max-h-[50vh] min-h-[40vh] lg:min-h-[50vh]'.split(' ');
  ...
  render() {
    this.innerHTML = `
      <div class="card-head flex justify-between border-b-2 py-1 dark:bg-gray-700 ${this.classList[1]} ${this.classList[2]}">
        <h2 class="text-3xl text-white">${this.dataset.title}</h2>
        <div class="flex flex-col my-auto">
          <p class="text-white text-sm underline">${this.dataset.subtitle}</p>
          <p class="text-white flex justify-end text-xs italic">${this.dataset.command}</p>
        </div>
      </div>
      <div id="${this.dataset.title.toLowerCase()}" class="${this.classList[3]} ${
      this.classList[4]
